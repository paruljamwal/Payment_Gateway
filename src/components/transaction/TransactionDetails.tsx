...
function TransactionDetailRows({
  ...
  const rows: { term: string; description: string }[] = [
    { term: "Transaction ID", description: transaction.id },
    { term: "Status", description: statusLabel },
    { term: "Amount", description: `${amountLabel} ${transaction.currency}` },
    {
      term: "Retry attempts",
  ...
  return (
    <SectionCard className="p-4 sm:p-5">
      <dl className="grid gap-4 text-sm leading-relaxed" aria-labelledby={labelledById}>
        {rows.map((row) => (
        ...
        {failureReason ? (
          <div className="grid gap-1 rounded-lg border border-amber-200 bg-amber-50 p-3 dark:border-amber-900 dark:bg-amber-950/40">
            <dt className="text-xs font-medium uppercase tracking-wide text-amber-900 dark:text-amber-100">
              Failure reason
            </dt>
